...
To target a specific environment directly (e.g. dev vs. test vs. prod) without manually switching profiles, pass `-Environment` to either script — see the examples below.
...
|---|---|---|---|
| `-Environment` | export, import | _(prompts interactively)_ | Target Power Platform environment URL (e.g. `https://bchydro-dev.crm.dynamics.com`). When omitted, `pac` uses the currently selected auth profile. |
| `-SolutionName` | export | `AIOnboardingSNReferenceSolution` | Name of the solution as it appears in Power Platform |
| `-ZipPath` | export, import | `.\solution.zip` | Path to the packaged solution zip (generated, not committed) |
...
# Export from a specific environment (skips the default-profile prompt)
./scripts/export.ps1 -Environment "https://bchydro-dev.crm.dynamics.com"
...
```powershell
# Default: packs .\solution into .\solution.zip and imports it into the current environment
./scripts/import.ps1

# Import into a specific environment (e.g. promoting from dev to test)
./scripts/import.ps1 -Environment "https://bchydro-test.crm.dynamics.com"
...
Tip: separating dev and test environments via `-Environment` makes it easy to promote a solution through environments without rewriting the script or changing your active `pac auth` profile.
...
The `solution/` folder is what `pac solution unpack` produces and is the source of truth committed to git. `bots/` and `botcomponents/` hold the Copilot Studio agent definition and its components; `Other/` contains solution-level metadata (`Solution.xml`, `Customizations.xml`, etc.).

`solution.zip` is **not** committed — it's a build artifact regenerated by both `export.ps1` and `import.ps1`. Keep this line in your `.gitignore`:
